...
def parse_row(row):
    ...
    row = {DATA_COLUMN_TO_FIELD[k]:v for k, v in row.items() if k in DATA_COLUMN_TO_FIELD.keys()}
...
def get_def_data(def_data):
    ...
    with open(def_data, encoding="cp1250") as f:
        for row in f:
            ...
            row = {DEF_COLUMN_TO_FIELD[k]:v for k, v in row.items() if k in DEF_COLUMN_TO_FIELD.keys()}
...
def main(filename, def_data):
    ...
    with open(filename, encoding="cp1250") as f:
        for i, row in enumerate(f):
            ...
            to_add.append(core.models.FoodNutrition(**parsed))
            # Bulk create every 5000 entries to save RAM
            if i % 5000 == 0:
                core.models.FoodNutrition.objects.bulk_create(to_add)
                to_add.clear()
    core.models.FoodNutrition.objects.bulk_create(to_add)
